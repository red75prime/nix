...
// /// requests.
// unsafe fn ptrace_get_data<T: Copy>(request: ptrace::PtraceRequest, pid: Pid) -> Result<T> {
//     let mut data = mem::MaybeUninit::<T>::uninit();
//     let res = unsafe { libc::ptrace(request, pid.into(), ptr::null_mut(), data.as_mut_ptr() as *mut c_void) };
//     Errno::result(res)?;
//     Ok(data.assume_init())
// }

// fn ptrace_other(request: ptrace::PtraceRequest, pid: Pid, addr: *mut c_void, data: *mut c_void) -> Result<c_long> {
//     Errno::result(unsafe { libc::ptrace(request, pid.into(), addr, data) }).map(|_| 0)
// }
...
//     let res = unsafe { libc::ptrace(PTRACE_SETOPTIONS, pid.into(), ptr::null_mut(), options as *mut c_void) };
...
//     let ret = unsafe{
//         Errno::clear();
//         libc::ptrace(PTRACE_SETSIGINFO, pid.into(), ptr::null_mut(), sig as *const _ as *const c_void)
//     };
//     match Errno::result(ret) {
//         Ok(_) => Ok(()),
//         Err(e) => Err(e),
